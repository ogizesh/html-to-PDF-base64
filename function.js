window.function = function (html, fileName, format, zoom, orientation, margin, breakBefore, breakAfter, breakAvoid, fidelity, customDimensions) {
    ...
    // Load html2pdf and ensure it's available
        ...
        try {
            ...
            html2pdf().set(opt).from(element).toPdf().output('datauristring')
                .then(function(pdfBase64) {
                    ...
                    return JSON.stringify(debugInfo); // Return base64String in JSON format
                })
                .catch(function(error) {
                    document.body.removeChild(element); // Clean up the DOM
                    debugInfo.message = 'Error generating PDF: ' + error.message;
                    return JSON.stringify(debugInfo); // Return error message in JSON format
                });
        } catch (error) {
            debugInfo.message = 'Caught error: ' + error.message;
            return JSON.stringify(debugInfo); // Return caught error in JSON format
        }
    };
